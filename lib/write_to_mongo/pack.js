...
function save_detail(detail, sector) {
    ...
    return function (callback) {
        ...
        var steps = [
            function (done) {
                console.log('writing points %s', detail);
                var file_name, query;
                ...
                stream.on('data', function (point) {
                    ...
                    try {
                        ...
                        console.log('error doing ej: %s', err);
                        throw err;
                    }

                    console.log('writing %s: %s to detail %s', point._id, j, detail);
        ...
        if ( !by_sector){
            steps.push(
                ...
                function (done) {
                    ...
                    stream.on('close', function () {
                        gate.await(function(){
                            console.log('done writing faces for %s', detail);
                            write_stream.end(']}', done);
                        });
                    })
