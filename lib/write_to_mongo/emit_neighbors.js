...
function emit_neighbors(done, min_depth, max_depth) {
    ...
    var scripts = [
        ...
        function (callback) {
            Point.update({detail: {$gte: min_depth, $lt: max_depth}}, {$set: {neighbors: []}}, {safe: true, multi: true}, callback);
        },
        ...
        function (done) {
            if (_DEBUG) console.log('assigning neighbors to points');
            var count = 0;
            ...
            Neighbor.find(query).count(function (err, total) {
                if (_DEBUG) console.log('neighbors at detail %s: %s', util.inspect(query), total);
                if (err) throw err;
                ...
                neighbor_stream.on('data', function (n) {
                    ...
                    n.set_point_neighbors(function (err, npoint) {
                        if (_DEBUG && !(count % 100)) {
                            if (_DEBUG) console.log('%s neighbors linked (%s neighbors for %s)',
                ...
                neighbor_stream.on('error', function (err) {
                    if (_DEBUG) console.log('error: %s', err);
                    gate.abort = true;
                    done(err);
                });
